...
To get the nutritional information of the food, all you need to do is upload the pictures of your meals to HealthNuts and Clarifai's API will return a set of predicted food items for each photo. Since Clarifai is not always correct, HealthNuts gives the power to the user by having them select the appropiate food items from the list of predictions sorted by highest probability. Each food item selected is then used to query nutritional information from the food database HealthNuts uses, Nutritionix. This information is not specific but is intending to give a generalized idea for people to understand what nutrients are highly present in each food item based on the daily recommended value. Users can also look back and view their history and can select each photo from their past meals to see the nutrients of each one.  

Going forward: HealthNuts plans to improve this web app by implementing more features in the near future. With the nutritional data accessed and present in the app, there comes alot more opportunity to help people become nutritionally aware. One of the main features that needs to be complete is Goals/Challenge. Once it is, the dashboard on the home page will show a high level sub component of the current goals or challenges unique to the user. Utilizing the nutrient info, users will be able to create a goal based on their needs i.e want to eat more or less protein for the next 2 weeks. HealthNuts will have a long list of challenges and a providing daily challenges like eat something that an elephant eats offers a fun approach to forming healthy eating habits. Displaying streaks is another essential feature that will be done at some point which will show each user patterns of what nutrients they consume most consistently so they can either maintain their eating habits or make the appropiate adjustements. In short, there is still work to be done but HealthNuts has paved a solid foundation for the future. With all this said, we hope you enjoy this revolutionary application because it is one of its kind.  
